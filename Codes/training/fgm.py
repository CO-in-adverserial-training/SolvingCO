...
def fgm(model, x, y, upper_limit, lower_limit, epsilon: float = 8/255, alpha: float = 1.0, k: float = 2.0, clip: bool = False, device: str = 'cuda'):
    ...
    if k != 0:
        eta = torch.randn_like(x).to(device)  # Gaussian noise
        eta_norm = torch.norm(eta.view(eta.shape[0], -1), p=2, dim=1).view(-1, 1, 1, 1)  # Compute L2 norm
        eta = k * eta / (eta_norm + 1e-8) * epsilon * torch.sqrt(torch.tensor(D, dtype=torch.float32))  # Scale to L2 norm epsilon * sqrt(D)
...
def train_epoch(model, optimizer, scheduler, loader, criterion, attack_params, eval_mode_attack: bool, device: str = 'cuda'):
    ...
    for img, lbl in loader:
        ...
        if eval_mode_attack:
            model.eval()
            delta, grad = fgm(model, img, lbl, **attack_params, device=device)
            model.train()
        else:
            delta, grad = fgm(model, img, lbl, **attack_params, device=device)
...
def train(architecture, loader, attack_params: dict, num_classes: int = 10, seed: int = 0, n_epochs: int = 30, eval_mode_attack: bool = False, device: str = 'cuda'):
    # Setting the seed
    np.random.seed(seed)
