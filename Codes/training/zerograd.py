...
def zero_grad(model, x, y, upper_limit, lower_limit, epsilon: float = 8/255, alpha: float = 2.0, q_val: float = 0.35, k: float = 1.0, clip: bool = True, device: str = 'cuda'):
...
def train_epoch(model, optimizer, scheduler, loader, criterion, attack_params, eval_mode_attack: bool, device: str = 'cuda'):
    ...
    for img, lbl in loader:
        ...
        if eval_mode_attack:
            model.eval()
            delta, grad = zero_grad(model, img, lbl, **attack_params, device=device)
            model.train()
        else:
            delta, grad = zero_grad(model, img, lbl, **attack_params, device=device)
...
def train(architecture, loader, attack_params: dict, eval_mode_attack: bool = False, num_classes: int = 10, seed: int = 0, n_epochs: int = 30, device: str = 'cuda'):
    ...
    if not attack_params['clip']:
        saving_dir += '_N-ZeroGrad_'
    else:
        saving_dir += '_ZeroGrad_'
