...
def get_loaders(args, index_dataset: bool, device):
    ...
    trainset = CIFAR10(root=f'{args.root_path}/{args.dataset}/data', train=True, download=True, transform=train_transform)
    trainset = IndexDataset(trainset) if index_dataset else trainset # Index Dataset

    testset = CIFAR10(root=f'{args.root_path}/{args.dataset}/data', train=False, download=True, transform=test_transform)
