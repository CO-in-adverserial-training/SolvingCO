...
# architectures/vit.py  (or similar)
import torch
import torch.nn as nn
import types
from huggingface_hub import hf_hub_download

def pretrained_vit_cifar10(num_classes=10,
                           repo_id="nickpai/vit-layer12-32-cifar10",
                           filename="vit-layer12-32-cifar10/vit-layer12-32-cifar10-199.pt"):
    """
    Load the custom CIFAR-10 ViT checkpoint and return a model whose forward(x) returns logits.
    The function expects the model class implementation (model.py) to be available in one of:
      - architectures.vit_model (recommended)
      - model (same directory)
    If you put the file somewhere else, adjust the import accordingly.
    """

    # 2) Instantiate the model with the hyperparameters from the model card
    ...
    # 3) Download checkpoint from HF hub
    weight_path = hf_hub_download(repo_id=repo_id, filename=filename)

    # 4) Load checkpoint safely (map to CPU)
    # Note: torch.load(..., weights_only=True) is suggested to reduce pickle risks in newer PyTorch,
    # but older PyTorch versions may not support the arg; we try with it and fallback.
    try:
        checkpoint = torch.load(weight_path, map_location="cpu", weights_only=True)
    except TypeError:
        checkpoint = torch.load(weight_path, map_location="cpu")

    # 5) Extract state dict from known wrappers
    if isinstance(checkpoint, dict):
        # common keys that wrap the state dict
    ...
    # 7) Load into model (allow partial load and report)
    missing, unexpected = model.load_state_dict(cleaned_state, strict=False)
    print("✅ Weights load attempt finished.")
    if missing:
        print(f"⚠️ Missing keys: {len(missing)} (example keys): {missing[:10]}")
    if unexpected:
        print(f"⚠️ Unexpected keys: {len(unexpected)} (example keys): {unexpected[:10]}")

    # 8) Patch forward to return logits only (model originally returns (logits, att_mat_full))
    # Save original forward implementation
    orig_forward = model.forward

    def logits_forward(self, x, *args, **kwargs):
        out = orig_forward(x, *args, **kwargs)
        # If original returns tuple (logits, att), return logits only.
        if isinstance(out, tuple) or isinstance(out, list):
            return out[0]
        # If it already returns logits (tensor), just return it
        return out

    model.forward = types.MethodType(logits_forward, model)

    print("[Info] pretrained_vit_cifar10: model ready. model.forward now returns logits (tensor).")
    return model
